...
Since dot product is a fairly decent measure of similarity, and faster to compute, Luong et al  proposed a dot product between encoder’s output and decoder’s previous hidden state as a basis for attention mechanism.  For this to work however, both of these vectors must be of the same dimensions.  This type of attention mechanism is known as multiplicative attention.  Just like in Concatenative attention, the dot product gives a score and all the scores (at a given decoder timestep) go through a softmax layer to give the final weights.  Another modification proposed was to use the decoder’s hidden state at the current time step rather than at the previous time step (i.e., ) rather than, , then to use the output from the attention mechanism (denoted as ) directly to compute the decoder's predictions (rather than using it to compute the decoder’s current hidden state). They also proposed a variant of the dot product mechanism where the encoder outputs first go through a linear transformation (i.e., a time-distributed Dense layer without a bias term) before the dot products are computed.  This is called the “general” dot product approach.  They compared both dot product approaches to the concatenative attention mechanism (adding a rescaling parameter v), and they observed that the dot product variants performed better than concatenative attention.
