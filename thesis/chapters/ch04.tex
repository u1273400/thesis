...
\centering
  ...
  \caption{Neuron cell \citep{landahl1943statistical} where $x_i$ are inputs and $w_i$ are input weights} \label{fig_3_1_ptron}
\end{figure}
...
\centering
  ...
  \caption{Perceptron algorithm having multiple neuron cells} \label{fig_3_2_nn}
\end{figure}
...
\centering
  ...
  \caption{An LSTM Cell showing Input, output and forget  gates  \citep{graves2013hybrid}}\label{fig_3_3_lstmcell}
\end{figure}
...
\centering
  ...
  \caption{Beam Search Lattice Structure showing forward (left) and backward (right) paths \citep{graves2006connectionist}}
...
where $z$ is the output label and $x$ is the input sequence.  Since $\mathcal{L}(S)$ in equation \ref{eqn_c3_ctc11} is differentiable, this loss function can be back propagated to the soft max layer in the BiRNN configuration discussed in section \ref{deepspeech}.
\begin{equation}
...
\centering
  ...
  \caption{Attention mechanism is centred around a time-distributed dense operation that determines similarity measure between current decoding sequence hidden input and all input sequence outputs.}
\label{fig_c4_attn}
\end{figure}
