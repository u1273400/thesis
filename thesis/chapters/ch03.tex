...
\centering
  % Requires \usepackage{graphicx}
  \includegraphics[width=9cm]{thesis/images/diags.jpeg}\\
  \caption{Types of Sequence-to-Sequence networks.  \citep{karathy2015unreasonable}\label{fig_c3_seq2seq}
\end{figure}

In the wild, three types of naturally occurring sequence relationships are observed, one-to-many also known as \acrfull{simo},  many-to-one also known as \acrfull{miso} and many-to-many also known as \acrfull{mimo}.  The one-to-one also referred to as \acrfull{siso} is excluded here.  Although \acrshort{siso} relationship is naturally existing, this is not a sequential relationship and will not be modelled using a \acrlong{rnn}, but rather, a regular \acrlong{dnn} will accurately model a \acrshort{siso} relationship.  In addition the \acrshort{mimo} relationship can be further divided
\stopblue
